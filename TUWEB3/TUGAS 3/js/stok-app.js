// Vue App untuk halaman Stok Bahan Ajar - TUGAS 3
...
var app = Vue.createApp({
  ...
  computed: {
    ...
    // Computed property untuk mendapatkan informasi lokasi aktif
    lokasiAktif() {
      return this.lokasiTab === 'pusat' ? 'Pusat' : 'UPBJJ';
    },
    
    // Computed property untuk mendapatkan jenis stok aktif
    jenisStokAktif() {
      const jenisMap = {
        'item': 'Stok Item',
        'paket': 'Stok Paket',
        'lokasi': 'Stok per Lokasi'
      };
      return jenisMap[this.jenisStok] || 'Stok Item';
    }
  },
  ...
  methods: {
    ...
    // Method untuk mendapatkan class badge berdasarkan stok
    getStokBadgeClass(stok) {
      if (stok > 100) return 'badge-success';
      if (stok > 50) return 'badge-info';
      if (stok > 20) return 'badge-warning';
      return 'badge-danger';
    }
  },
  ...
  watch: {
    ...
    // Watcher untuk memantau perubahan totalStok (computed property)
    totalStok(newValue, oldValue) {
      if (newValue !== oldValue) {
        console.log(`Total stok berubah dari ${oldValue} menjadi ${newValue}`);
      }
    }
  },
